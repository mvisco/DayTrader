...
public class GraphSeries extends ApplicationFrame  {
    ...
    public GraphSeries(final String title) {

        super(title);
        
        final CombinedDomainXYPlot plot = new CombinedDomainXYPlot(new DateAxis("Time"));
        
        this.datasets = new XYSeriesCollection[SUBPLOT_COUNT+1];
        ...
        for (int i = 0; i < SUBPLOT_COUNT; i++) {
			...
			if (i==0) {
				...
				final XYSeries series2 = new XYSeries("Mean");
				...
				// volume sub-plot
				final XYSeries series = new XYSeries("Volume");
				this.datasets[2] = new XYSeriesCollection(series);
    ...
    public  void updateDatasets (double price, double mean, int volume, int time) {
        ...
        for (int i = 0; i < SUBPLOT_COUNT; i++) {
            ...
            if (i==0) {              
                ...
                this.datasets[0].getSeries(0).add(time, this.lastValue[0]);  
                this.lastValue[1] = mean;
                this.datasets[1].getSeries(0).add(time, this.lastValue[1]);
                
            
            }
            else {
            	this.volValue = volume;
                this.datasets[2].getSeries(0).add(time, this.volValue);
            }
        }   	
