...
public class ScalpingSimulation {
	...
	public static void main(String[] args) {
		...
		String initialDate = "2017-4-2";
		String finalDate = "2017-4-9";
		Date iDate=null;
		try {
		...
		for (Stock stock : listOfStocks ) {
			...
			if (stock.getStrategyID() == 0) {
			   ...
			   stock.setStrategy(strategy);
				...
				Strategy strategy = new StockGammaScalping(stock.getSymbol(), stock.getLot(), stock.getChange(),
						stock.getTradeable(), account, stock.getLoss(), stock.getProfit(), stock.getImpVol(), stock.getVolumeVector());
				stock.setStrategy(strategy);
		...
		for (int j =0; j < initialDayIndex.size(); j++ )  {
			    ...
			    for (Level1Quote newQuote : l) {
			    	...
			    	if ((i >= (l.size() - 200)) && (j == initialDayIndex.size() - 1)) {
			    		// Close all positions we are in the last 200 quotes and at the end of the cycle
			    		lastQuote = newQuote;
			    		if ( (newQuote == null) || newQuote.getSymbol().contentEquals("") ) {
			    			continue;
			    		}
