	...
	public void stateTransition(Level1Quote quote) {
	   ...
	   double change_from_previous_close;
		...
		case S0:
		    ...
		    } else if ( ( ((max - min) >= (this.objective_change*currentPrice)) &&(currentBid <= (min )) )
		...
		case S1:
			...
			// close position if we made "profit" or we lost "loss"
				...
				if ( (currentBid <= (this.price-this.profit*currentPrice)) ) {
					if (display) {
				    ...
				    clearMean();
				} else if ((currentBid >= (this.price+(2*this.objective_change)*currentPrice)) ){
					if (display) {
		...
		case S2: 
			...
			// close position if we made "profit" or we lost "loss"
				...
				if (currentAsk >= (this.price+(this.profit*currentPrice))) {
					if (display) {
		...
		case S3:
			...
			if ( (currentAsk <= this.price-this.profit*currentPrice) || (currentAsk >= (this.price+(this.loss*currentPrice)))
					) {	
		...
		case S4:
			 ...
			 if ( (currentBid >= (this.price+this.profit*currentPrice)) || (currentBid <= (this.price - (this.loss*currentPrice))) 
					 ){
