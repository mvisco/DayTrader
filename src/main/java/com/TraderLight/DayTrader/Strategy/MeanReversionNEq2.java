	...
	public void stateTransition(Level1Quote quote) {
       ...
       if ( (hours >= 14) ) {
		...
		if ( (count % 20) == 0 ) {
		   ...
		   log.info("Position value is " + account.getPortfolioValue(quote, this));
           log.info("        ");
		}
		...
		case S1:
		    ...
		    } else if (currentBid >= (mean+(2*objective_change))) {
		...
		case S2: 
            ...
            } else if (currentAsk <= (mean-(2*objective_change))) {
		...
		case SDelta:
			...
			double value = account.getPortfolioValue(quote, this);
		
			if ( value > 1000 ) {
				
				log.info("Selling everything for symbol " + symbol);
				log.info("Portfolio Value is " + value);				
				this.currentState = States.SDelta;
				this.desiredState=States.S0;
				this.StrategyState=States.STemp;
				account.getPortfolioValue(quote, this);
				account.sellOption(quote, symbol, this, SELL);
				clearMean();
...
/*		
			...
			if (delta <= -2500) {
				...
				//account.sellDeltaAdjustment(quote, symbol, this, quantity*100, "P", this);
				account.sellDeltaAdjustment(quote, quote.getSymbol(), this, SELL, quantity, "P");
				
			//} else if (delta >= (0.5*calls)) {
			} else if (delta >= (2500)) {
				// sell delta calls 
				//quantity = delta_int;
				quantity = 2500;
		...
		switch (StrategyState) {
		   ...
		   /*
		     ...
		     if (symbol.contentEquals("MSFT")) {
				...
				// account.buy_or_sell(BUY, OPEN, lastQuote,2*new_lot*100 , this);
		   ...
		   */
		    ...
		    /*
		    	if (symbol.contentEquals("MSFT")) {
		    		...
		    		// account.buy_or_sell(SELL, OPEN, lastQuote, 2*new_lot*100, this);
