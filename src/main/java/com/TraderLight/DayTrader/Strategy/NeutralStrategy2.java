...
import com.TraderLight.DayTrader.Strategy.NeutralStrategy.States;
...
public class NeutralStrategy2 extends Strategy{
	
		
	enum States {S0, S1, S2, S3, S4, STemp, SInit1, SInit2, SInit3, SInit4 };
	...
	public NeutralStrategy2(String symbol, int symbol_lot, double change, boolean isTradeable, AccountMgr account, double loss, 
	   ...
	   double change_from_previous_close;
		...
		case S0:
			...
			// or if there is a manual  intervention that tells us to open a long position
				...
				this.currentState = States.S0;
				this.desiredState=States.SInit2;
				this.StrategyState = States.STemp;
		...
		case SInit2: 
			log.info("State SInit2):  attempting to buy one lot of puts for symbol: " + quote.getSymbol() + " at bid " + currentBid);
			this.currentState = States.SInit1;
		...
		case SInit3:
			...
			log.info("State SInit3):  attempting to buy one lot of calls for symbol: " + quote.getSymbol() + " at bid " + currentBid);
			this.currentState = States.SInit3;
		...
		case SInit4:
			log.info("State SInit2):  attempting to buy one lot of puts for symbol: " + quote.getSymbol() + " at bid " + currentBid);
		    this.currentState = States.SInit4;
		...
		case S1:
			...
			/*
					...
					this.currentState = States.S1;
					this.desiredState=States.SInit3;
					this.StrategyState=States.STemp;
		...
		case S2: 
			...
			// close position if we made "profit" or we lost "loss"
				...
				if (currentAsk >= (this.price+this.profit)) {
					...
					this.currentState = States.S2;
					this.desiredState=States.SInit4;
					this.StrategyState = States.STemp;
