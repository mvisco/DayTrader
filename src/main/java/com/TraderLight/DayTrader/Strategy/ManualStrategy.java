...
public class ManualStrategy extends Strategy{
	...
	public ManualStrategy(String symbol, int symbol_lot, double change, boolean isTradeable, AccountMgr account, double loss, 
	   ...
	   double change_from_previous_close;
		...
		if ( (count % 10) == 0 ) {
		    ...
		    log.info("Bid : " + Math.round(currentBid*100)/100.0 + " ,Ask : " + Math.round(currentAsk*100)/100.0 +" ; Mean is: " + Math.round(mean*100)/100.0 + " ;difference is " + 
		         (Math.round(currentPrice*100)/100.0-Math.round(mean*100)/100.0) + " ; objective_change is " + this.objective_change);
		    log.info("volume  ratio is " + ((double)currentVolume/(double)avgVolume) + " ;change from previous close is " + change_from_previous_close + " ;lot is " +
		            this.lot);	
		    log.info("profit is " + profit + " loss is  " + loss);
		...
		case S0:
			...
			// We open a position if there is a manual  intervention that tells us to open a short position
			
		    if ( (openShortPositionWithPrice && (currentBid >= shortPositionPrice)) ){
	
				log.info("State S0: attempting to open position, sell one lot of stocks for symbol: " + quote.getSymbol());
				this.currentState = States.S0;
			...
			// We open a position if there is a manual  intervention that tells us to open a long position
		    			    	
		    } else if ( (openLongPositionWithPrice && (currentAsk <= longPositionPrice)) ) {
		    		
				log.info("State S0 attempting to open position, buy  one lot of stocks for symbol: " + quote.getSymbol());
				this.currentState = States.S0;
		...
		case S1:
            ...
            if ( (currentAsk <= this.price-this.profit) || (currentAsk >= (this.price+this.loss)) ) {	
				...
				// if flag has not been reset go to SNull so we avoid potential trading loops
				if (openShortPositionWithPrice) {
					this.desiredState=States.SNull;
				} else {
					this.desiredState=States.S0;
				}
				this.StrategyState = States.STemp;
		...
		case SNull:
			if (!openLongPositionWithPrice && !openShortPositionWithPrice ) {
				// move to S0 otherwise stay here
				this.StrategyState = States.S0;
			}
		case STemp:
